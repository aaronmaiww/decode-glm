...
def load_token_dataframes(
    base_path: str, subset_indices: Tuple = (0,)
) -> Dict[str, Any]:
    ...
    for idx in subset_indices:
        ...
        try:
            ...
            raise FileNotFoundError(f"Token dataframe not found at {path}") from err

    return {"dataframes": dataframes, "sequence_ids": sequence_ids}
...
def get_model_activations(
    ...
    Args:
        ...
        flatten (bool): Whether to flatten the output activations
    ...
    total_batches = (
        n_seqs + batch_size - 1
    ) // batch_size  # round up (n_seqs / batch_size)
    ...
    with torch.no_grad():
        ...
        for i in range(0, n_seqs, batch_size):
            ...
            batch_attention_mask = tokens["attention_mask"][i : i + batch_size].to(
                device
            )

            # Get MLP-outputs of model at layer_N
            mlp_act = utils.get_layer_activations(
                model, batch_input_ids, batch_attention_mask, layer_N=layer_num
            )
            ...
            if flatten:
                mlp_act = mlp_act.reshape(
                    -1, model.config.hidden_size
                )  # reshape tensor(batch_s, seq_len, hidden_d) to tensor(batch_s*seq_len, hidden_d)
                mlp_act = mlp_act.detach().cpu()  # move to CPU
...
def prepare_probe_inputs(
    ...
    activations = get_model_activations(
        ...
        flatten=flatten,  # flatten to shape (n_tokens, hidden_dim)
    )
    # activations is a tensor of shape (n_tokens, hidden_dim) if flatten=True,
    # or (n_sequences, seq_len, hidden_dim) if flatten=False
...
def get_seq_ids_from_token_data(
    token_data: Dict, subset_indices: Tuple[int, ...]
) -> List[str]:
    """
    ...
    all_subset_ids = []
    for _i, subset_idx in enumerate(subset_indices):
...
def create_binary_labels(
    df: pd.DataFrame, annotation_column: str, annotations: List[str], kmer: str = None
) -> pd.Series:
...
def compute_class_statistics(labels: pd.Series) -> Dict[str, float]:
    ...
    if positive_count == 0:
        return {"positive_count": 0, "base_rate_accuracy": None, "class_frequency": 0}
...
def prepare_tensors(
    features: torch.Tensor, labels: pd.Series
    ...
    x_tensor = (
        features.clone().detach().to(dtype=torch.float32)
    )  # also convert to float32, in case of mixed precision
...
def process_annotation(
    ...
    annotation_column: str = "token_annotations",
) -> Dict[str, Any]:
    ...
    return {"features": x_tensor, "labels": y_tensor, "statistics": stats}


def generate_shuffled_label_matrix(
    original_labels: torch.Tensor, num_probes: int, real_probe_indices: list = None
) -> torch.Tensor:
    ...
    assert len(original_labels) == len(real_probe_indices), (
        "Number of label sets must match number of real probe indices"
    )
...
class BatchProbeTrainer:
    def __init__(
        self, num_probes: int, input_dim: int, config: dict, loss_weights=None
    ):
        ...
        if loss_weights is not None:
            ...
            self.criterion = nn.BCEWithLogitsLoss(
                pos_weight=loss_weights.to(self.device)
            )
        else:
            self.criterion = nn.BCEWithLogitsLoss()
    ...
    def create_data_loaders(
        self, x_tensor: torch.Tensor, y_tensor_matrix: torch.Tensor
    ...
    def evaluate(self, val_loader) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_f1": sum(m["f1"] for m in metrics)
            / len(metrics),  # Keep for backward compatibility
            "best_f1": best_f1,
    ...
    def train(
        ...
        for epoch in range(self.config["num_epochs"]):
            ...
            if verbose and epoch % 5 == 0:  # Show detailed metrics every 5 epochs
                ...
                print(
                    f"Avg F1: {metrics['avg_f1']:.4f}, Best F1: {metrics['best_f1']:.4f}"
                )
        ...
        if verbose:
            print(
                f"Training complete. Best F1: {best_f1_score:.4f} (Probe {best_metrics['best_probe_idx']})"
            )
