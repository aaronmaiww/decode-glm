...
# from interplm.constants import DATA_DIR  # Not needed for local implementation
DATA_DIR = "data"
import torch
...
# Import trainer configs after other components to avoid circular imports
from .trainers.base_trainer import SAETrainerConfig
from .trainers.batch_top_k import BatchTopKTrainerConfig
from .trainers.jump_relu import JumpReLUTrainerConfig
from .trainers.relu import ReLUTrainerConfig
from .trainers.top_k import TopKTrainerConfig
from .wandb_manager import WandbConfig

# Import crosscoder config
try:
    from crosscoder.trainer import CrossCoderTrainerConfig
except ImportError:
    CrossCoderTrainerConfig = None
    print("Warning: CrossCoderTrainerConfig not available")
...
def _get_trainer_config_class(trainer_data: dict) -> type:
    ...
    if "shared_ratio" in trainer_data and CrossCoderTrainerConfig is not None:
        return CrossCoderTrainerConfig
    elif "l1_penalty" in trainer_data:
        ...
        print(
            "Warning: Could not determine trainer type, defaulting to ReLUTrainerConfig"
        )
        return ReLUTrainerConfig
...
class TrainingRunConfig:
    ...
    def _sync_normalization_params(self):
        ...
        if self.eval_cfg.eval_embd_dir is not None:
            ...
            if (
                self.eval_cfg.zscore_means_file is None
                and self.dataloader_cfg.zscore_means_file is not None
            ):
                self.eval_cfg.zscore_means_file = self.dataloader_cfg.zscore_means_file
                print(
                    f"📋 Auto-synced eval zscore_means_file: {self.eval_cfg.zscore_means_file}"
                )

            # Copy zscore_vars_file if not set in eval config
            if (
                self.eval_cfg.zscore_vars_file is None
                and self.dataloader_cfg.zscore_vars_file is not None
            ):
                self.eval_cfg.zscore_vars_file = self.dataloader_cfg.zscore_vars_file
                print(
                    f"📋 Auto-synced eval zscore_vars_file: {self.eval_cfg.zscore_vars_file}"
                )

            # Copy target_dtype if not explicitly set
            if (
                hasattr(self.dataloader_cfg, "target_dtype")
                and self.dataloader_cfg.target_dtype != torch.float32
            ):
    ...
    def from_yaml(cls, config_path: Union[str, Path]) -> "TrainingRunConfig":
        ...
        def _convert_str_to_special_types(obj, template=None):
            if template is not None and hasattr(template, "__dataclass_fields__"):
                ...
                for field_name, field_info in template.__dataclass_fields__.items():
                    ...
                    if (
                        field_info.type is not None
                        and "dtype" in field_name
        ...
        if "dataloader_cfg" in data:
            data["dataloader_cfg"] = _convert_str_to_special_types(
                data["dataloader_cfg"], DataloaderConfig
            )
        ...
        if "trainer_cfg" in data:
            ...
            data["trainer_cfg"] = _convert_str_to_special_types(
                data["trainer_cfg"], trainer_config_class
            )
            data["trainer_cfg"] = trainer_config_class(**data["trainer_cfg"])

        if "eval_cfg" in data:
            data["eval_cfg"] = _convert_str_to_special_types(
                data["eval_cfg"], EvaluationConfig
            )
            data["eval_cfg"] = EvaluationConfig(**data["eval_cfg"])

        if "wandb_cfg" in data:
            data["wandb_cfg"] = _convert_str_to_special_types(
                data["wandb_cfg"], WandbConfig
            )
            data["wandb_cfg"] = WandbConfig(**data["wandb_cfg"])

        if "checkpoint_cfg" in data:
            data["checkpoint_cfg"] = _convert_str_to_special_types(
                data["checkpoint_cfg"], CheckpointConfig
            )
