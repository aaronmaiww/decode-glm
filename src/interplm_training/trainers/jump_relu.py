...
from .common import get_lr_schedule, get_sparsity_warmup_fn
...
class JumpReLUTrainer(SAETrainer):
    ...
    def loss(self, x: torch.Tensor, step: int, logging=False, **_):
        ...
        active_indices = (f.sum(0) > 0).to(self.device)
        did_fire = torch.zeros_like(self.num_tokens_since_fired, dtype=torch.bool, device=self.device)
...
class StepFunction(autograd.Function):
    ...
    def backward(ctx, grad_output):
        x, threshold, bandwidth_tensor = ctx.saved_tensors
        bandwidth = bandwidth_tensor.item()
        x_grad = torch.zeros_like(x, device=x.device)
        threshold_grad = (
            -(1.0 / bandwidth)
            * RectangleFunction.apply((x - threshold) / bandwidth)
            * grad_output
