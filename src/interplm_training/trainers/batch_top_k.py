...
from interplm_sae.dictionary import (
    BatchTopKSAE,
    remove_gradient_parallel_to_decoder_directions,
    set_decoder_norm_to_unit_norm,
)
from interplm_training.utils import get_device

from .base_trainer import SAETrainer, SAETrainerConfig
from .common import get_autocast_context, get_lr_schedule
...
class BatchTopKTrainerConfig(SAETrainerConfig):
    ...
    def __post_init__(self):
        # Set expansion_factor if activation_dim is provided
        if self.activation_dim is not None:
            self.set_and_validate_activation_dim(self.activation_dim)
...
class BatchTopKTrainer(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        f, active_features_mask, post_relu_acts_BF = self.ae.encode(
        ...
        did_fire[active_features_mask] = True
        self.num_tokens_since_fired += num_tokens_in_step
        self.num_tokens_since_fired[did_fire] = 0

        # Update steps since active
        self.steps_since_active += 1
        self.steps_since_active[active_features_mask] = 0
