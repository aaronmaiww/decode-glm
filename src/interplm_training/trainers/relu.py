...
from interplm_sae.dictionary import ReLUSAE
        ...
        with t.no_grad():
            ...
            if activations.shape[0] >= n_dead:
                ...
                sampled_activations = activations[
                    t.randint(0, activations.shape[0], (n_dead,))
                ]

            # Reinitialize encoder weights for dead neurons
            # Use a small random perturbation of the sampled activations
            noise_scale = 0.01
            new_encoder_weights = sampled_activations + noise_scale * t.randn_like(
                sampled_activations
            )
            self.ae.encoder.weight.data[dead_neurons] = (
                new_encoder_weights / new_encoder_weights.norm(dim=1, keepdim=True)
            )
            ...
            self.ae.decoder.weight.data[:, dead_neurons] = self.ae.encoder.weight.data[
                dead_neurons
            ].T
