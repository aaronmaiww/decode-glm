...
class ZScoreUnnormalizer:
    ...
    def _load_zscore_stats(self, zscore_means_file: Path, zscore_vars_file: Path):
        ...
        print(
            f"Loading z-score unnormalization stats from {zscore_means_file} and {zscore_vars_file}"
        )
        ...
        if zscore_means_file.suffix == ".pt":
            ...
            with open(zscore_means_file) as f:
        ...
        if zscore_vars_file.suffix == ".pt":
            ...
            # Assume text file with one number per line
            with open(zscore_vars_file) as f:
        ...
        print(
            f"Loaded z-score means shape: {self.means.shape}, vars shape: {vars_data.shape}, computed stds shape: {self.stds.shape}"
        )
...
class SingleShardActivationLoader(Dataset):
    ...
    def permutation(self):
        ...
        if self._permutation is None:
            self._permutation = (
                torch.randperm(self.total_tokens)
                if self.shuffle
                else torch.arange(self.total_tokens)
            )
...
class ShardedActivationsDataset(Dataset):
    ...
    def _load_shard_info(self, subdir: Path):
        ...
        if (subdir / "metadata.json").exists():
            ...
            metadata = yaml.load(open(subdir / "metadata.yaml"), Loader=yaml.FullLoader)
        ...
        if active_shard_index != self._last_active_shard:
            for i, dataset_info in enumerate(self.datasets):
                ...
                if hasattr(dataset, "_close_memmap"):
                    ...
                    if (
                        not should_keep_loaded
                        and hasattr(dataset, "_is_loaded")
                        and dataset._is_loaded
                    ):
                        dataset._close_memmap()
...
class ActivationsDataLoader(DataLoader):
    def __init__(
        ...
        if (
            self.config.zscore_means_file is not None
            and self.config.zscore_vars_file is not None
        ):
