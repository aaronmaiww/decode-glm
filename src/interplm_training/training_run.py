...
from .configs import TrainingRunConfig
from .data_loader import ActivationsDataLoader
from .evaluation import EvaluationManager
from .trainers.base_trainer import SAETrainer
...
class SAETrainingRun:
                ...
                if self.wandb_manager._should_log(step):
                    ...
                    if hasattr(self.trainer, "steps_since_active"):
                        information_to_log["features/dead_100_steps"] = (
                            (self.trainer.steps_since_active > 100).sum().item()
                        )

                        information_to_log["features/dead_1000_steps"] = (
                            (self.trainer.steps_since_active > 1000).sum().item()
                        )
    ...
    def _finalize_training(self):
        ...
        if hasattr(self.trainer, "get_per_dimension_mse"):
            try:
                ...
                if (
                    self.evaluation_manager.eval_activations is not None
                    and len(self.evaluation_manager.eval_activations) > 0
                ):
                ...
                if self.wandb_manager.use_wandb:
                    self.wandb_manager.log_metrics(
                        final_metrics, self.training_state["current_step"]
                    )
                    print(
                        f"Logged final per-dimension MSE stats - Mean: {final_metrics['final/per_dim_mse_mean']:.6f}"
                    )
    ...
    def _run_final_evaluation(self):
        ...
        # Check if we're using a fidelity config before importing
        try:
            from .fidelity import ESMFidelityConfig
        except ImportError:
            print("\nSkipping final comprehensive evaluation (ESM not available)")
            return
        ...
        try:
            ...
            final_fidelity = self.evaluation_manager._calculate_fidelity(
                self.trainer.ae
            )
