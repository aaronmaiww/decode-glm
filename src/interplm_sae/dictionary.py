...
class Dictionary(ABC, nn.Module):
    ...
    def has_normalization_factors(self) -> bool:
        ...
        return not t.allclose(
            self.activation_rescale_factor, t.ones_like(self.activation_rescale_factor)
        )
    ...
    def _unnormalize_output(self, x_normalized, original_norms):
        ...
        if self.normalize_to_sqrt_d:
            ...
            sqrt_d = t.sqrt(
                t.tensor(d, dtype=x_normalized.dtype, device=x_normalized.device)
            )
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        ...
        destination[prefix + "normalize_to_sqrt_d"] = t.tensor(
            self.normalize_to_sqrt_d, dtype=t.bool
        )
...
class ReLUSAE(Dictionary):
    ...
    def from_pretrained(path, device=None):
        ...
        autoencoder = ReLUSAE(
            activation_dim, dict_size, normalize_to_sqrt_d=normalize_to_sqrt_d
        )
...
class TopKSAE(Dictionary, nn.Module):
    ...
    def __init__(
        self, activation_dim: int, dict_size: int, k: int, normalize_to_sqrt_d=False
    ):
        super().__init__(normalize_to_sqrt_d)
        self.activation_dim = activation_dim
        self.dict_size = dict_size
    ...
    def from_pretrained(path, k: Optional[int] = None, device=None):
        ...
        autoencoder = TopKSAE(
            activation_dim, dict_size, k, normalize_to_sqrt_d=normalize_to_sqrt_d
        )
        autoencoder.load_state_dict(state_dict)
        if device is not None:
            autoencoder = autoencoder.to(device)
...
class JumpReLUSAE(Dictionary, nn.Module):
    ...
    def __init__(
        self, activation_dim, dict_size, device="cpu", normalize_to_sqrt_d=False
    ):
        super().__init__(normalize_to_sqrt_d)
        self.activation_dim = activation_dim
        self.dict_size = dict_size
...
class BatchTopKSAE(Dictionary, nn.Module):
    ...
    def forward(
        self, x: t.Tensor, output_features: bool = False, unnormalize: bool = False
    ):
        """
        Forward pass of an autoencoder.
    ...
    def from_pretrained(
        ...
        if threshold is None:
            ...
            assert threshold == state_dict["threshold"].item(), (
        ...
        autoencoder = BatchTopKSAE(
            ...
            dict_size=dict_size,
            k=k,
            normalize_to_sqrt_d=normalize_to_sqrt_d,
        ).to(device=device)
        autoencoder.load_state_dict(state_dict)
        if device is not None:
