...
# )
# Note: These are not available in our local implementation
...
def calculate_feature_statistics(
    ...
    print(
        f"  Features that were non-zero at least once: {sum(is_feature_ever_nonzero)}"
    )
...
def normalize_sae_features(
    ...
    norm_sae_path = sae_dir / "ae_normalized.pt"
    ...
    max_per_feat = calculate_feature_statistics(
        sae=sae, data_loader=data_loader, n_shards=n_shards
    )
